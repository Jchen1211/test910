...
#'
#' @param count A gene count matrix or Seurat object. Before input the count data,it is recommended to finish the imputation of count matrix.
...
#' @param input The format of input count.Default: matrix. Seurat: input Seurat object.
...
#' @examples
#' count_filtered <- genefiltered(count, frac = 0.01, nfeatures = 2000,
#'                                    input = "matrix", mt.percent = 5)
genefiltered <- function(count, frac = 0.01, nfeatures = 2000, input = "matrix", mt.percent = 5){
  if(input == "Seurat"){
    obj <- GetAssayData(count, slot = "counts")
  }
  else {
    obj <- count
  }
